...
tf.flags.DEFINE_string("result_log", "../../results/1202_red_vs_others/result.txt", "path to mPA log")
tf.flags.DEFINE_integer("batch_size", "16", "batch size for training")
tf.flags.DEFINE_string("logs_dir", "../../results/1202_red_vs_others/checkpoints/", "path to logs directory")
tf.flags.DEFINE_string("vis_dir",  "../../results/1202_red_vs_others/vis/", "path to save results of visualization")
tf.flags.DEFINE_string("vis_train_dir",  "../../results/1202_red_vs_others/vis_train/", "path to save results of visualization")
...
prob_log = "../../results/1202_red_vs_others/prob.txt"
...
def main(argv=None):
    ...
    val_loss_sum = tf.summary.scalar("val_loss", loss)
    ...
    if FLAGS.mode == "train":
        ...
        for itr in xrange(MAX_ITERATION):
            ...
            if itr % 50 == 0:
                train_loss, summary_str, summary_loss = sess.run([loss, summary_op, train_loss_sum], feed_dict=feed_dict)
                print("Step: %d, Train_loss:%g" % (itr, train_loss))
                summary_writer.add_summary(summary_str, itr)
                summary_writer.add_summary(summary_loss, itr)

            if (itr % 5000 == 0):
                valid_images, valid_annotations = validation_dataset_reader.next_batch(FLAGS.batch_size, random_rotate = 0)
                valid_loss, summary_loss = sess.run([loss, val_loss_sum], feed_dict={image: valid_images, annotation: valid_annotations,
                ...
                test_loss, summary_loss = sess.run([loss, test_loss_sum], feed_dict={image: test_images, annotation: test_annotations, keep_probability: 1.0})
                summary_writer.add_summary(summary_loss, itr)
