...
tf.flags.DEFINE_string("result_log", "../../results/1210_red_vs_others_quarter/result.txt", "path to mPA log")
tf.flags.DEFINE_integer("batch_size", "16", "batch size for training")
tf.flags.DEFINE_string("logs_dir", "../../results/1210_red_vs_others_quarter/checkpoints/", "path to logs directory")
tf.flags.DEFINE_string("vis_dir",  "../../results/1210_red_vs_others_quarter/vis/", "path to save results of visualization")
tf.flags.DEFINE_string("vis_train_dir",  "../../results/1210_red_vs_others_quarter/vis_train/", "path to save results of visualization")
...
prob_log = "../../results/1210_red_vs_others_quarter/prob.txt"
...
def Gray2BGR(gray_img):
    tmp_layer = tf.ones_like(gray_img) * 255
    B_layer = tf.zeros_like(gray_img)
    G_layer = tf.zeros_like(gray_img)
    R_layer = tf.zeros_like(gray_img)
    B_layer = tf.where(tf.not_equal(gray_img, 3), B_layer, tmp_layer)
    G_layer = tf.where(tf.not_equal(gray_img, 1), G_layer, tmp_layer)
    R_layer = tf.where(tf.not_equal(gray_img, 2), R_layer, tmp_layer)
    ret = tf.concat([R_layer, G_layer, B_layer], axis=3)
    return ret
...
def main(argv=None):
    ...
    labels = tf.where(tf.not_equal(labels, 3), labels, tf.ones_like(labels) * 1)
    ...
    gt_sum = tf.summary.image("ground_truth", tf.cast(annotation_BGR, tf.uint8), max_outputs=4)
    ...
    unweighted_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=logits, name="entropy")
    weighted_loss = unweighted_loss * weights
    loss = tf.reduce_mean(weighted_loss)

    loss_sum = tf.summary.scalar("loss", loss)
    # lr_sum = tf.summary.scalar("learning_rate", tf.constant(FLAGS.learning_rate))
    lr_sum = tf.summary.scalar("learning_rate", tf.Variable(FLAGS.learning_rate, tf.float64))
    ...
    # get quarter dataset
    train_records = train_records[0:int(len(train_records)/4)]
    ...
    summary_train_writer = tf.summary.FileWriter(FLAGS.logs_dir + "/train/", sess.graph)
    summary_val_writer = tf.summary.FileWriter(FLAGS.logs_dir + "/val/", sess.graph)
    summary_test_writer = tf.summary.FileWriter(FLAGS.logs_dir + "/test/", sess.graph)
    ...
    if FLAGS.mode == "train":
        ...
        for itr in xrange(MAX_ITERATION):
            ...
            if itr % 50 == 0:
                train_loss, summary_str, summary_loss = sess.run([loss, summary_op, loss_sum], feed_dict=feed_dict)
            ...
            if (itr % 2000 == 0):
                ...
                test_images, test_annotations = test_dataset_reader.next_batch(FLAGS.batch_size, random_rotate = 0)
                test_loss, summary_loss = sess.run([loss, loss_sum], feed_dict={image: test_images, annotation: test_annotations, keep_probability: 1.0})
                ...
                for i in range(len(test_records)):
                    ...
                    # merge label 1,3 to label 1
                    test_annotations = np.where(test_annotations != 3, test_annotations, 1)
        ...
        for itr in range(len(test_records)):
            ...
            pred = sess.run(pred_annotation, feed_dict={image: test_images, annotation: test_annotations,
                                                        keep_probability: 1.0})
            costMap = sess.run(costProb, feed_dict={image: test_images, annotation: test_annotations,
                                                        keep_probability: 1.0})
            test_annotations = np.squeeze(test_annotations, axis=3)
            pred = np.squeeze(pred, axis=3)
