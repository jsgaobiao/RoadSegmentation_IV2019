...
string CONFIG_FILE = "../ProjectVelo2Video/config.ini";
string FILE_LIST;
string DATA_PATH;
string PRED_PATH;
...
void Init() {
    ...
    START_TIME = fileName[0].toInt();
    END_TIME = fileName[fileName.size() - 1].toInt();
...
void LabelUnknown(cv::Mat &gtImg, cv::Mat &input)
{
    for (int i = 0; i < gtImg.rows; i ++) {
        for (int j = 0; j < gtImg.cols; j ++) {
            // Unknown pixel
            if (input.at<cv::Vec3b>(i, j)[0] + input.at<cv::Vec3b>(i, j)[1] + input.at<cv::Vec3b>(i, j)[2] == 0) {
                gtImg.at<cv::Vec3b>(i, j)[0] = 0;
                gtImg.at<cv::Vec3b>(i, j)[1] = 0;
                gtImg.at<cv::Vec3b>(i, j)[2] = 0;
            }
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    for (int idx = 0; idx < fileName.size(); idx++) {
        ...
        // Read image data
        inputImg = cv::imread(DATA_PATH + fileName[idx].toStdString() + "_img.png", cv::IMREAD_COLOR);
        videoImg = cv::imread(DATA_PATH + fileName[idx].toStdString() + "_video.png", cv::IMREAD_COLOR);
        inputImgForVis = inputImg.clone();
